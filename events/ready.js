...
module.exports = async (bot) => {
  ...
  try {
    bot.channels.cache.get(botConfig.loggingChannel).send(botConfig.startupMessage.replace("vX", `v${removeEndingZeroes(version)}`));
  }
  catch (err) {
    console.error("Couldn't send the start-up message to the log channel!\n", err);
  }
}
